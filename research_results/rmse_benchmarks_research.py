...
def run_comprehensive_benchmark():
    ...
    for size in sizes:
        ...
        mean_time, std_time = benchmark_function(
            rmse_numpy_manual, y_true_np, y_pred_np
        )
        ...
        if PYTORCH_AVAILABLE:
            ...
            mean_time, std_time = benchmark_function(
                rmse_pytorch_cpu, y_true_torch, y_pred_torch
            )
            ...
            if torch.cuda.is_available():
                mean_time, std_time = benchmark_function(
                    rmse_pytorch_gpu, y_true_torch, y_pred_torch
                )
        ...
        if TENSORFLOW_AVAILABLE:
            ...
            mean_time, std_time = benchmark_function(
                rmse_tensorflow_cpu, y_true_tf, y_pred_tf
            )
            ...
            if tf.config.list_physical_devices("GPU"):
                mean_time, std_time = benchmark_function(
                    rmse_tensorflow_gpu, y_true_tf, y_pred_tf
                )
    ...
    for dtype in dtypes:
        ...
        if NUMBA_AVAILABLE:
            time_numba, _ = benchmark_function(
                rmse_numba_basic, y_true, y_pred, n_runs=50
            )
    ...
    if PYTORCH_AVAILABLE and torch.cuda.is_available():
        ...
        for size in sizes_gpu:
            ...
            time_cpu, _ = benchmark_function(
                rmse_pytorch_cpu, y_true_torch, y_pred_torch, n_runs=20
            )

            # GPU time
            time_gpu, _ = benchmark_function(
                rmse_pytorch_gpu, y_true_torch, y_pred_torch, n_runs=20
            )
