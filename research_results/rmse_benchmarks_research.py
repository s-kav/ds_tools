...
def rmse_pytorch_gpu(y_true: torch.Tensor, y_pred: torch.Tensor) -> float:
    """RMSE in PyTorch on GPU"""
    if torch.cuda.is_available():
        y_true_gpu = y_true.cuda()
        y_pred_gpu = y_pred.cuda()
        result = torch.sqrt(torch.mean((y_true_gpu - y_pred_gpu) ** 2))
        return result.cpu().item()
    else:
        return rmse_pytorch_cpu(y_true, y_pred)

# 2. TensorFlow implementation
def rmse_tensorflow_cpu(y_true: tf.Tensor, y_pred: tf.Tensor) -> float:
    """RMSE in TensorFlow on CPU"""
    with tf.device('/CPU:0'):
        return tf.sqrt(tf.reduce_mean(tf.square(y_true - y_pred))).numpy()

def rmse_tensorflow_gpu(y_true: tf.Tensor, y_pred: tf.Tensor) -> float:
    """RMSE in TensorFlow on GPU"""
    if tf.config.list_physical_devices('GPU'):
        with tf.device('/GPU:0'):
            return tf.sqrt(tf.reduce_mean(tf.square(y_true - y_pred))).numpy()
    else:
        return rmse_tensorflow_cpu(y_true, y_pred)

# 3. Scikit-learn implementation
def rmse_sklearn(y_true: np.ndarray, y_pred: np.ndarray) -> float:
    """RMSE using sklearn"""
...
if NUMBA_AVAILABLE:
    ...
    def rmse_numba_basic(y_true: np.ndarray, y_pred: np.ndarray) -> float:
        """Basic Numba implementation RMSE"""
        return np.sqrt(np.mean((y_true - y_pred) ** 2))
    
    @jit(nopython=True, parallel=True, fastmath=True)
    def rmse_numba_parallel(y_true: np.ndarray, y_pred: np.ndarray) -> float:
        """Parallel Numba implementation RMSE"""
        n = len(y_true)
        total = 0.0
        for i in prange(n):
    ...
    @jit(nopython=True, fastmath=True)
    def rmse_numba_vectorized(y_true: np.ndarray, y_pred: np.ndarray) -> float:
...
def rmse_numpy_manual(y_true: np.ndarray, y_pred: np.ndarray) -> float:
    """Manual implementation without built-in functions"""
    n = len(y_true)
    total = np.sum((y_true - y_pred) ** 2)
    return np.sqrt(total / n)
...
def run_comprehensive_benchmark():
    ...
    print("🚀 COMPREHENSIVE BENCHMARK OF RMSE IMPLEMENTATIONS")
    print("=" * 80)
    ...
    for size in sizes:
        ...
        if PYTORCH_AVAILABLE:
            ...
            mean_time, std_time = benchmark_function(rmse_pytorch_cpu, y_true_torch, y_pred_torch)
            size_results['PyTorch CPU'] = mean_time
            print(f"PyTorch CPU:     {mean_time*1000:.3f}ms ± {std_time*1000:.3f}ms")
            
            if torch.cuda.is_available():
                mean_time, std_time = benchmark_function(rmse_pytorch_gpu, y_true_torch, y_pred_torch)
                size_results['PyTorch GPU'] = mean_time
                print(f"PyTorch GPU:     {mean_time*1000:.3f}ms ± {std_time*1000:.3f}ms")
        ...
        if TENSORFLOW_AVAILABLE:
            ...
            mean_time, std_time = benchmark_function(rmse_tensorflow_cpu, y_true_tf, y_pred_tf)
            size_results['TensorFlow CPU'] = mean_time
            print(f"TensorFlow CPU:  {mean_time*1000:.3f}ms ± {std_time*1000:.3f}ms")
            
            if tf.config.list_physical_devices('GPU'):
                mean_time, std_time = benchmark_function(rmse_tensorflow_gpu, y_true_tf, y_pred_tf)
                size_results['TensorFlow GPU'] = mean_time
                print(f"TensorFlow GPU:  {mean_time*1000:.3f}ms ± {std_time*1000:.3f}ms")
...
def plot_results(results: Dict):
    ...
    plt.title('RMSE Execution Time vs Data Size')
    plt.legend()
    ...
    colors = ['green' if v < 1 else 'orange' if v < 1.2 else 'red' for v in efficiency_values]
    for bar, color in zip(bars, colors):
        bar.set_color(color)
...
def detailed_analysis():
    ...
    for dtype in dtypes:
        ...
        # NumPy
        time_np, _ = benchmark_function(rmse_numpy, y_true, y_pred, n_runs=50)
        print(f"  NumPy:     {time_np*1000:.3f}ms")
        
        # Numba
        if NUMBA_AVAILABLE:
            time_numba, _ = benchmark_function(rmse_numba_basic, y_true, y_pred, n_runs=50)
    ...
    if NUMBA_AVAILABLE:
        ...
        start_time = time.perf_counter()
        rmse_numba_basic(y_true, y_pred)
    ...
    if PYTORCH_AVAILABLE and torch.cuda.is_available():
        ...
        sizes_gpu = [1000000, 10000000, 100000000]
        ...
        for size in sizes_gpu:
            ...
            # CPU time
            time_cpu, _ = benchmark_function(rmse_pytorch_cpu, y_true_torch, y_pred_torch, n_runs=20)
            
            # GPU time
            time_gpu, _ = benchmark_function(rmse_pytorch_gpu, y_true_torch, y_pred_torch, n_runs=20)
...
def main():
    ...
    recommendations = """
    📊 GENERAL RMSE PERFORMANCE RECOMMENDATIONS:
