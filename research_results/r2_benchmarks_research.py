...
def run_comprehensive_benchmark():
    ...
    for size in sizes:
        ...
        print(
            f"NumPy:           {mean_time*1000:.3f}ms ± {std_time*1000:.3f}ms → R²: {result:.6f}"
        )

        # NumPy manual
        mean_time, std_time, result = benchmark_function(
            r2_numpy_manual, y_true_np, y_pred_np
        )
        size_results["NumPy Manual"] = mean_time
        accuracy_info["NumPy Manual"] = result
        print(
            f"NumPy Manual:    {mean_time*1000:.3f}ms ± {std_time*1000:.3f}ms → R²: {result:.6f}"
        )

        # NumPy optimized
        mean_time, std_time, result = benchmark_function(
            r2_numpy_optimized, y_true_np, y_pred_np
        )
        size_results["NumPy Optimized"] = mean_time
        accuracy_info["NumPy Optimized"] = result
        print(
            f"NumPy Optimized: {mean_time*1000:.3f}ms ± {std_time*1000:.3f}ms → R²: {result:.6f}"
        )

        # Sklearn
        if SKLEARN_AVAILABLE:
            mean_time, std_time, result = benchmark_function(
                r2_sklearn, y_true_np, y_pred_np
            )
            size_results["Sklearn"] = mean_time
            accuracy_info["Sklearn"] = result
            print(
                f"Sklearn:         {mean_time*1000:.3f}ms ± {std_time*1000:.3f}ms → R²: {result:.6f}"
            )

        # Numba implementation
        if NUMBA_AVAILABLE:
            mean_time, std_time, result = benchmark_function(
                r2_numba_basic, y_true_np, y_pred_np
            )
            size_results["Numba Basic"] = mean_time
            accuracy_info["Numba Basic"] = result
            print(
                f"Numba Basic:     {mean_time*1000:.3f}ms ± {std_time*1000:.3f}ms → R²: {result:.6f}"
            )

            mean_time, std_time, result = benchmark_function(
                r2_numba_parallel, y_true_np, y_pred_np
            )
            size_results["Numba Parallel"] = mean_time
            accuracy_info["Numba Parallel"] = result
            print(
                f"Numba Parallel:  {mean_time*1000:.3f}ms ± {std_time*1000:.3f}ms → R²: {result:.6f}"
            )

            mean_time, std_time, result = benchmark_function(
                r2_numba_vectorized, y_true_np, y_pred_np
            )
            size_results["Numba Vector"] = mean_time
            accuracy_info["Numba Vector"] = result
            print(
                f"Numba Vector:    {mean_time*1000:.3f}ms ± {std_time*1000:.3f}ms → R²: {result:.6f}"
            )
        ...
        if PYTORCH_AVAILABLE:
            ...
            mean_time, std_time, result = benchmark_function(
                r2_pytorch_cpu, y_true_torch, y_pred_torch
            )
            size_results["PyTorch CPU"] = mean_time
            accuracy_info["PyTorch CPU"] = result
            print(
                f"PyTorch CPU:     {mean_time*1000:.3f}ms ± {std_time*1000:.3f}ms → R²: {result:.6f}"
            )

            if torch.cuda.is_available():
                mean_time, std_time, result = benchmark_function(
                    r2_pytorch_gpu, y_true_torch, y_pred_torch
                )
                size_results["PyTorch GPU"] = mean_time
                accuracy_info["PyTorch GPU"] = result
                print(
                    f"PyTorch GPU:     {mean_time*1000:.3f}ms ± {std_time*1000:.3f}ms → R²: {result:.6f}"
                )
        ...
        if TENSORFLOW_AVAILABLE:
            ...
            mean_time, std_time, result = benchmark_function(
                r2_tensorflow_cpu, y_true_tf, y_pred_tf
            )
            size_results["TensorFlow CPU"] = mean_time
            accuracy_info["TensorFlow CPU"] = result
            print(
                f"TensorFlow CPU:  {mean_time*1000:.3f}ms ± {std_time*1000:.3f}ms → R²: {result:.6f}"
            )

            if tf.config.list_physical_devices("GPU"):
                mean_time, std_time, result = benchmark_function(
                    r2_tensorflow_gpu, y_true_tf, y_pred_tf
                )
                size_results["TensorFlow GPU"] = mean_time
                accuracy_info["TensorFlow GPU"] = result
                print(
                    f"TensorFlow GPU:  {mean_time*1000:.3f}ms ± {std_time*1000:.3f}ms → R²: {result:.6f}"
                )
...
def plot_results(results: Dict, accuracy_results: Dict):
    ...
    if accuracy_results and any(accuracy_results.values()):
        ...
        for method in methods:
            ...
            for size in sizes:
                if (
                    size in accuracy_results
                    and method in accuracy_results[size]
                    and ref_method in accuracy_results[size]
                    and accuracy_results[size][method] is not None
                    and accuracy_results[size][ref_method] is not None
                ):

                    error = abs(
                        accuracy_results[size][method]
                        - accuracy_results[size][ref_method]
                    )
            ...
            if method_errors:
                ...
                print(
                    f"Method {method}: {len(method_errors)} data points, min error: {min(method_errors):.2e}"
                )
        ...
        if all_accuracy_data:
            ...
            for i, (method, (method_sizes, method_errors)) in enumerate(
                all_accuracy_data.items()
            ):
                plt.plot(
                    method_sizes,
                    method_errors,
            ...
            if all_errors:
                ...
                plt.text(
                    0.02,
                    0.98,
                    f"Error range: {min_error:.2e} - {max_error:.2e}",
    ...
    if large_sizes:
        ...
        if numpy_manual_times:
            ...
            numpy_manual_times_list = [
                numpy_manual_times[m] for m in numpy_manual_methods
            ]

            # Добавляем точку для NumPy Manual
            ax2.plot(
                len(methods_sorted) + 0.5,
                numpy_manual_times_list[0],
                "ro",
                markersize=10,
                label="NumPy Manual",
            )
...
def detailed_analysis():
    ...
    for dtype in dtypes:
        ...
        # Numba
        if NUMBA_AVAILABLE:
            time_numba, _, result_numba = benchmark_function(
                r2_numba_basic, y_true, y_pred, n_runs=50
            )
            speedup = time_np / time_numba
            print(
                f"  Numba:     {time_numba*1000:.3f}ms (speedup: {speedup:.2f}x) → R²: {result_numba:.6f}"
            )
    ...
    if PYTORCH_AVAILABLE and torch.cuda.is_available():
        ...
        for size in sizes_gpu:
            ...
            # CPU time
            time_cpu, _, result_cpu = benchmark_function(
                r2_pytorch_cpu, y_true_torch, y_pred_torch, n_runs=20
            )

            # GPU time
            time_gpu, _, result_gpu = benchmark_function(
                r2_pytorch_gpu, y_true_torch, y_pred_torch, n_runs=20
            )
