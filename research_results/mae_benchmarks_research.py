...
def run_comprehensive_benchmark():
    ...
    for size in sizes:
        ...
        if NUMBA_AVAILABLE:
            mean_time, std_time = benchmark_function(mae_numba_basic, y_true_np, y_pred_np)
            size_results['Numba Basic'] = mean_time
            print(f"Numba Basic:     {mean_time*1000:.3f}ms ± {std_time*1000:.3f}ms")
            
            mean_time, std_time = benchmark_function(mae_numba_parallel, y_true_np, y_pred_np)
            size_results['Numba Parallel'] = mean_time
            print(f"Numba Parallel:  {mean_time*1000:.3f}ms ± {std_time*1000:.3f}ms")
            
            mean_time, std_time = benchmark_function(mae_numba_vectorized, y_true_np, y_pred_np)
        ...
        if PYTORCH_AVAILABLE:
            ...
            mean_time, std_time = benchmark_function(mae_pytorch_cpu, y_true_torch, y_pred_torch)
            size_results['PyTorch CPU'] = mean_time
            print(f"PyTorch CPU:     {mean_time*1000:.3f}ms ± {std_time*1000:.3f}ms")
            
            if torch.cuda.is_available():
                mean_time, std_time = benchmark_function(mae_pytorch_gpu, y_true_torch, y_pred_torch)
        ...
        if TENSORFLOW_AVAILABLE:
            ...
            mean_time, std_time = benchmark_function(mae_tensorflow_cpu, y_true_tf, y_pred_tf)
            size_results['TensorFlow CPU'] = mean_time
            print(f"TensorFlow CPU:  {mean_time*1000:.3f}ms ± {std_time*1000:.3f}ms")
            
            if tf.config.list_physical_devices('GPU'):
                mean_time, std_time = benchmark_function(mae_tensorflow_gpu, y_true_tf, y_pred_tf)
...
def detailed_analysis():
    ...
    for dtype in dtypes:
        ...
        # Numba
        if NUMBA_AVAILABLE:
            time_numba, _ = benchmark_function(mae_numba_basic, y_true, y_pred, n_runs=50)
    ...
    if PYTORCH_AVAILABLE and torch.cuda.is_available():
        ...
        for size in sizes_gpu:
            ...
            # CPU time
            time_cpu, _ = benchmark_function(mae_pytorch_cpu, y_true_torch, y_pred_torch, n_runs=20)
            
            # GPU time
            time_gpu, _ = benchmark_function(mae_pytorch_gpu, y_true_torch, y_pred_torch, n_runs=20)
