...
if NUMBA_AVAILABLE:
    ...
    def _euclidean_numba(u, v):
        dist_sq = 0.0
        for i in range(u.shape[0]): dist_sq += (u[i] - v[i])**2
        return np.sqrt(dist_sq)
if CUPY_AVAILABLE:
    def _euclidean_cupy(u, v): return cp.linalg.norm(u - v)
def _euclidean_numpy(u, v): return np.linalg.norm(u - v)
...
if NUMBA_AVAILABLE:
    ...
    def _manhattan_numba(u, v):
        dist = 0.0
        for i in range(u.shape[0]): dist += abs(u[i] - v[i])
        return dist
if CUPY_AVAILABLE:
    def _manhattan_cupy(u, v): return cp.sum(cp.abs(u - v))
def _manhattan_numpy(u, v): return np.sum(np.abs(u - v))
...
if NUMBA_AVAILABLE:
    ...
    def _minkowski_numba(u, v, p):
        dist = 0.0
        for i in range(u.shape[0]): dist += abs(u[i] - v[i])**p
        return dist**(1.0 / p)
if CUPY_AVAILABLE:
    def _minkowski_cupy(u, v, p): return cp.sum(cp.abs(u - v)**p)**(1.0 / p)
def _minkowski_numpy(u, v, p): return np.sum(np.abs(u - v)**p)**(1.0 / p)
...
if NUMBA_AVAILABLE:
    ...
    def _chebyshev_numba(u, v):
        ...
        return max_dist
if CUPY_AVAILABLE:
    def _chebyshev_cupy(u, v): return cp.max(cp.abs(u - v))
def _chebyshev_numpy(u, v): return np.max(np.abs(u - v))
...
if CUPY_AVAILABLE:
    def _hamming_cupy(u, v): return cp.mean(u != v)
def _hamming_numpy(u, v): return np.mean(u != v)
...
if NUMBA_AVAILABLE:
    ...
    def _jaccard_numba(u, v):
        ...
        for i in range(u.shape[0]):
            u_bool = u[i] != 0
            v_bool = v[i] != 0
            if u_bool and v_bool: intersection += 1
            if u_bool or v_bool: union += 1
...
class Distance:
    def __init__(self, gpu_threshold: int = 10_000):
        ...
        print(f"Distance initialized. GPU: {self.gpu_available}, Numba: {self.numba_available}")

    def _validate_vectors(self, u, v, check_dims=True):
        if check_dims and (u.ndim != 1 or v.ndim != 1): raise ValueError("Inputs must be 1D vectors.")
        if u.shape[-1] != v.shape[-1]: raise ValueError("Input vectors/matrices must have the same number of features (last dimension).")
        if u.shape[0] == 0: return 0.0
        return None
    ...
    def _dispatch_v2v(self, name: str, u: np.ndarray, v: np.ndarray, force_cpu: bool):
        ...
        if use_gpu: u_c, v_c = cp.asarray(u_c), cp.asarray(v_c)
        return func, u_c, v_c
    ...
    def minkowski(self, u: np.ndarray, v: np.ndarray, p: int, force_cpu: bool = False):
        if p < 1: raise ValueError("p must be at least 1 for Minkowski distance.")
        if self._validate_vectors(u, v) is not None: return 0.0
    ...
    def cosine_similarity(self, u: np.ndarray, v: np.ndarray, force_cpu: bool = False):
        if self._validate_vectors(u, v) is not None: return 1.0
        func, u_c, v_c = self._dispatch_v2v("cosine_similarity", u, v, force_cpu)
        return float(func(u_c, v_c))

    def mahalanobis(self, u: np.ndarray, v: np.ndarray, cov_inv: np.ndarray, force_cpu: bool = False):
        if self._validate_vectors(u, v) is not None: return 0.0
        if cov_inv.ndim != 2 or cov_inv.shape[0] != cov_inv.shape[1] or cov_inv.shape[0] != u.shape[0]:
            raise ValueError("Inverse covariance matrix must be square with dimensions matching the vectors.")
    ...
    def haversine(self, lat1: float, lon1: float, lat2: float, lon2: float, radius: float = 6371.0):
    ...
    def pairwise_euclidean(self, X: np.ndarray, Y: Optional[np.ndarray] = None, force_cpu: bool = False):
        Y = X if Y is None else Y
        if self._validate_vectors(X, Y, check_dims=False) is not None: return np.array([[]])
        
        func, X_c, Y_c, use_gpu = self._dispatch_m2m("pairwise_euclidean", X, Y, force_cpu)
        if func is None: # Fallback to numpy if no specialized func
            return self._dispatch_m2m("pairwise_euclidean", X, Y, force_cpu=True)[0](X_c, Y_c)
            
        distances = func(X_c, Y_c)
        if use_gpu: return cp.asnumpy(distances)
        return distances

    def kmeans_distance(self, X, centroids, force_cpu: bool = False):
        return self.pairwise_euclidean(X, centroids, force_cpu=force_cpu)

    def knn_distances(self, X: np.ndarray, Y: Optional[np.ndarray] = None, k: int = 5, force_cpu: bool = False):
        Y = X if Y is None else Y
        if self._validate_vectors(X, Y, check_dims=False) is not None: return (np.array([]), np.array([]))
    ...
    def radius_neighbors(self, X: np.ndarray, Y: Optional[np.ndarray] = None, radius: float = 1.0, force_cpu: bool = False):
        Y = X if Y is None else Y
        if self._validate_vectors(X, Y, check_dims=False) is not None: return ([], [])
