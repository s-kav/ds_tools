...
def test_backend_dispatching_logic(tools, mocker, small_sample_vectors, large_sample_vectors):
...
def test_vector_distances_correctness(tools, method_name, kwargs, trusted_func, large_sample_vectors):
...
def test_mahalanobis_correctness(tools, small_sample_vectors, inverse_covariance_matrix):
...
def test_kmeans_distance_is_alias(tools, mocker, sample_matrices):
    ...
    mock_pairwise = mocker.patch.object(tools.distance, "pairwise_euclidean", return_value=np.array([]))
