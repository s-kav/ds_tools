...
def test_kl_divergence_quality(tools):
    kl_good = tools.calculate_kl_divergence(P, Q1)
    kl_bad = tools.calculate_kl_divergence(P, Q2)
    assert (
        kl_good < kl_bad
    ), "KL divergence from good approx should be lower than from bad approx"


def test_kl_base_flag_effect(tools):
...
def test_kl_normalization(tools):
    kl_norm = tools.calculate_kl_divergence(P, Q1)
    kl_unnorm = tools.calculate_kl_divergence(P_unnormalized, Q1_unnormalized)
    assert np.isclose(
        kl_norm, kl_unnorm
    ), "Unnormalized and normalized distributions must yield same KL"


def test_kl_raises_on_length_mismatch(tools):
    with pytest.raises(
        ValueError, match="Input distributions P and Q must have the same shape."
    ):
        tools.calculate_kl_divergence([0.5, 0.5], [0.3, 0.3, 0.4])


def test_kl_raises_on_negative_values(tools):
